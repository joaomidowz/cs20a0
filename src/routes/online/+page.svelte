<script lang="ts">
  ...
  async function loadCollectionOutcome(key: string) {
    ...
    collectionOutcome = { rank: null, points: 0, majorsWon: 0, awards: [], result: null, room: [], pending: true, missing: false };
    ...
    try {
      ...
      collectionOutcome = { rank: season.me?.rank ?? null, points: season.me?.points ?? 0, majorsWon: season.me?.majorsWon ?? 0, awards: awards.awards.slice(0, 8), result, room, pending: false, missing: !result };
  ...
  onMount(() => {
    ...
    const visibility = () => {
      if (document.hidden) { stopOfflineSounds(); return; }
      // A background tab throttles the reward poll's timers to ~1 a minute: catch up the moment the player returns.
      if (snapshot?.phase === 'completed' && collectionOutcome?.pending) {
        collectionOutcomeFor = '';
        void loadCollectionOutcome(`${roomCode}:${snapshot.season?.run ?? 0}`);
      }
    };
  ...
  /** Boost de farm: one daily activation resolves 10-20 solo majors server-side; coins at the solo rate, zero points. */
  let boost: BoostState | null = null;
  let boostTried = false;
  let boostBusy = false;
  let boostField: 'random' | 'champions' = 'random';
  let boostError = '';
  $: if ($accountUser && hasSavedLineup && !boostTried) { boostTried = true; void fetchBoost(getOnlineServerUrl()).then((state) => boost = state).catch(() => {}); }
  async function activateBoostRuns(extra: boolean) {
    if (boostBusy) return;
    if (extra && !await confirmDialog({
      title: t('boostTitle'),
      body: `${t('boostRun').replace('{n}', String((boost?.baseRuns ?? 10) + (boost?.extraRuns ?? 10)))} · ${(boost?.extraPrice ?? 3000).toLocaleString($language)} coins`,
      confirmLabel: t('buy'),
      cancelLabel: t('cancel')
    })) return;
    boostBusy = true; boostError = '';
    try { boost = await activateBoost(getOnlineServerUrl(), boostField, extra); }
    catch { boostError = t('connectionFailed'); }
  ...
  /** Sair de verdade: manda o `leave` ao servidor antes de fechar o socket, para a sala se resolver na hora. */
  function leaveRoom() {
    send({ type: 'leave' });
    exitRoom();
  }

  /** Botão de sair do header: com run em andamento, o primeiro clique arma e o segundo confirma. */
  let leaveArmed = false;
  let leaveArmTimer: ReturnType<typeof setTimeout> | undefined;
  function requestLeave() {
    const inFlight = Boolean(snapshot && snapshot.phase !== 'lobby' && snapshot.phase !== 'completed');
    if (!inFlight || leaveArmed) { leaveRoom(); return; }
    leaveArmed = true;
    clearTimeout(leaveArmTimer);
    leaveArmTimer = setTimeout(() => { leaveArmed = false; }, 3000);
  }
  ...
  /** Power (e coach do banco) de cada organização da run, para a tabela do suíço mostrar todos os times. */
  $: organizationPowerByOrg = snapshot?.organizations
    ? new Map(snapshot.organizations.map((organization) => [organization.id, {
        power: formatCourtRating(organization.power),
        coach: organization.coachId ? collectionCoachById.get(organization.coachId)?.name ?? null : null
      }]))
    : null;
...
<PageLayout wide language={$language} theme={$theme} onLanguage={(value) => $language = value} onTheme={() => $theme = $theme === 'dark' ? 'light' : 'dark'}>
    ...
    <section class="online-entry">
      ...
      <div class="mode-choice">
        ...
        {#if $accountUser && hasSavedLineup}
          ...
          <article class="panel mode-card boost-mode">
            <span class="eyebrow">{t('boostNoPoints')}</span>
            <h2>{t('boostTitle')}</h2>
            <p>{t('boostHint')}</p>
            <div class="boost-field" role="group" aria-label={t('boostTitle')}>
              <button type="button" class:active={boostField === 'random'} disabled={boostBusy || boost?.activated} on:click={() => boostField = 'random'}>{t('soloRandom')}</button>
              <button type="button" class:active={boostField === 'champions'} disabled={boostBusy || boost?.activated} on:click={() => boostField = 'champions'}>{t('soloChampions')}</button>
            </div>
            {#if boost?.activated}
              <p class="boost-done" role="status">{t('boostSummary').replace('{n}', String(boost.runs)).replace('{c}', boost.coins.toLocaleString($language)).replace('{t}', String(boost.titles))}</p>
              <small>{t('boostBack')}</small>
            {:else}
              <button class="secondary" type="button" disabled={boostBusy} on:click={() => void activateBoostRuns(false)}>{t('boostRun').replace('{n}', String(boost?.baseRuns ?? 10))}</button>
              <button class="primary" type="button" disabled={boostBusy} title={t('boostExtraHint')} on:click={() => void activateBoostRuns(true)}>{t('boostRun').replace('{n}', String((boost?.baseRuns ?? 10) + (boost?.extraRuns ?? 10)))} · {(boost?.extraPrice ?? 3000).toLocaleString($language)}</button>
  ...
  {:else}
    <section class="online-room">
      <header class:online-draft-header={snapshot.phase === 'draft'} class:online-header-compact={Boolean(snapshot.tournament) && snapshot.phase !== 'draft'} class="online-header">
        <!-- No Major em andamento o match-topbar já é o header: aqui sobra só o código da sala e o Sair, sem título duplicado. -->
        {#if !snapshot.tournament || snapshot.phase === 'draft'}
        ...
        <div class="room-actions">
          ...
          <button type="button" class="secondary leave-button" class:armed={leaveArmed} on:click={requestLeave}>{leaveArmed ? t('leaveConfirm') : `✕ ${t('leave')}`}</button>
        </div>
      </header>
      ...
      {:else if snapshot.phase === 'draft' && self}
        ...
        {/if}
        <section class="online-progress"><h2>{t('participants')}</h2>{#each snapshot.participants as participant}<article><span>{participant.organizationName}</span><b>{participant.picksCompleted}/5{#if participant.power !== null} · PWR {formatCourtRating(participant.power)}{/if} · {participant.mapsConfirmed ? '3/3' : '0/3'} {t('mapsConfirmed').toUpperCase()}</b><i role="progressbar" aria-label={participant.organizationName} aria-valuemin="0" aria-valuemax="100" aria-valuenow={participant.ready ? 100 : Math.min(90, participant.picksCompleted * 14 + (participant.mapsConfirmed ? 20 : 0))}><em style={`width:${participant.ready ? 100 : Math.min(90, participant.picksCompleted * 14 + (participant.mapsConfirmed ? 20 : 0))}%`}></em></i></article>{/each}</section>
        ...
        <section class="screen match-screen online-major-screen">
          ...
          {#if snapshot.phase === 'completed' && collectionOutcome}
            <section class="panel collection-outcome">
              ...
              {:else if collectionOutcome.missing}
                <!-- O servidor guarda a run só quando ela termina nele: um restart no meio e não há o que mostrar. -->
                <div class="earned"><h3>{t('youEarned')}</h3><p class="note">{t('resultMissing')}</p></div>
              {/if}
          ...
          {#if snapshot.phase === 'completed'}
            <div class="after-run">
              <button class="secondary" type="button" on:click={leaveRoom}>← {t('backToStart')}</button>
                    ...
                    <div class="timeline-phase completed">
                      ...
                      <div class="timeline-phase-content">
                        {#each matches as match}
                          ...
                          {#if expandedTimelineMatch === match.id}<div class="timeline-maps">{#each match.maps as map}<span class="timeline-map">{getMapName(map.mapId, map.map, gameT('map'))} · {map.scoreA} x {map.scoreB} {map.overtime ? '· OT' : ''}</span>{/each}</div>{/if}
                        {/each}
                      </div>
                    </div>
                  {/if}
          ...
          {:else}
            <MajorOverview tournament={onlineTournament} cursor={onlineCursor} userTeamId={me?.id ?? ''} language={$language} powerByOrg={organizationPowerByOrg} onTeam={openOrganization} onSeries={snapshot.phase === 'completed' ? null : (id) => watchSeries(id)} showLiveScores />
          {/if}
        </section>
      {/if}
      {#if errorMessage}<p class="online-error" role="alert">{errorMessage}</p>{/if}

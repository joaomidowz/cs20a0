<script lang="ts">
  ...
  import { DEFAULT_ROOM_CONFIG, type PublicOrganization, type PublicOverviewSeries, type RoomConfig, type RoomSnapshot } from '$lib/game/online/contracts';
  ...
  {:else}
    <section class="online-room">
      ...
      {#if snapshot.phase === 'lobby'}
        <div class="lobby-grid">
          ...
          <section class="panel room-settings">
            ...
            <h2>{t('title')}</h2>
            <label><span>Modo</span><select value={config.mode} disabled={!isHost} on:change={(event) => saveConfig({ mode: (event.currentTarget as HTMLSelectElement).value as RoomConfig['mode'] })}><option value="premier">Normal</option><option value="faceit">Ranked</option><option value="pro">PRO</option></select></label>
      ...
      {:else if snapshot.phase === 'draft' && self}
        ...
        {:else}
          <DraftHud selectedPlayers={displayLineup as SelectedPlayer[]} style={self.style ?? 'balanced'} styleLocked={Boolean(self.style)} styleLabel={gameT((self.style ?? 'balanced') as OrgStyle)} mode={snapshot.config.mode} revealed={me?.ready ?? false} label={gameT('orgHud')} onOpen={(player) => detailsPlayer = player} />
        {/if}
...
<OrganizationRosterModal
  ...
  language={$language}
  showPlayerAwards={snapshot ? shouldShowPlayerAwards(snapshot.config.mode, 'game') : false}

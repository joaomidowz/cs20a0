<script lang="ts">
  ...
  import { LEGAL_CONTACT, termsOfUse } from '$lib/legal';

  $: t = (key: Parameters<typeof translate>[1]) => translate($language, key);
  $: doc = termsOfUse($language);
</script>

<SeoHead metadata={SEO_BY_ROUTE['/terms']} />
...
>
  ...
  <div class="page-contact">
    <p>{t('termsContact')} <a href={`mailto:${LEGAL_CONTACT}`}>{LEGAL_CONTACT}</a> · <a href="/suporte">{$language === 'en' ? 'Support' : $language === 'es' ? 'Soporte' : 'Suporte'}</a></p>
  </div>
</PageLayout>

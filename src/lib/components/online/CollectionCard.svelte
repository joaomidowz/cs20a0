...
<article class="card rarity-{rarity}" class:compact class:in-lineup={inLineup} class:star class:up={effect === 'up'} class:down={effect === 'down'}>
  ...
  <button class="face" type="button" on:click={() => onOpen?.(player)} disabled={!onOpen}>
    ...
    <span class="role">{getRoleLabel(primaryRoleOf(player))} · {player.year ?? '—'}</span>

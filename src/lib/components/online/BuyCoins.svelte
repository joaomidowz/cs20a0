<script lang="ts">
  ...
  onMount(async () => {
    try {
      const result = await authFetch<{ products: Array<{ id: string; coins: number; price_cents?: number; priceCents?: number; currency: string }> }>(serverUrl, '/shop/products', { method: 'GET' });
      products = result.products.map((item) => ({ id: item.id, coins: item.coins, priceCents: item.priceCents ?? item.price_cents ?? 0, currency: item.currency }));
    } catch {
  ...
  async function buy(product: Product) {
    ...
    try {
      ...
      error = caught instanceof AccountError && caught.code === 'PURCHASE_LIMIT' ? text.limit : text.fail;
      busy = '';
    }
  }
</script>

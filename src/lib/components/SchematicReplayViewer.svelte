<script lang="ts">
  ...
  function startQueuedRound(roundNumber: number, _pendingBefore: number) {
    ...
    clock.setPlaybackWindow(roundStartMs, roundEndMs);
    ...
    clock.scrub(playbackSpeed === 'simulate' ? roundStartMs : roundStartMs + 1);
  ...
  function syncVisibleReplay(durationMs: number, roundCount: number) {
    ...
    if (autoplay && autoplayPlanId !== plan.id) {
      autoplayPlanId = plan.id;
      completedPlanId = '';
      liveQueue.resetToLatest(Math.min(1, roundCount), arrivalTimeMs());
  ...
  function scheduleBoundaryAdvance() {
    ...
    boundaryFrame = requestAnimationFrame(() => {
      ...
      if (renderedRoundEnd !== playbackRound) {
        ...
        if (autoplay) {
          if (!roundEndRendered) return;
          renderedRoundEnd = playbackRound;
          if (autoplay && playbackRound < receivedRounds) {
            startQueuedRound(playbackRound + 1, receivedRounds - playbackRound);
          } else if (playbackRound >= plan.rounds.length && receivedRounds >= plan.rounds.length) {
            notifyComplete();
          }
          return;
        }
      ...
      else if (autoplay && playbackRound >= plan.rounds.length && receivedRounds >= plan.rounds.length) {
        notifyComplete();
      }
    });
  }
...
<section class="replay-viewer" aria-label={t('replay')}>
  ...
  <div class="replay-controls">
    ...
    <div class="replay-speed" aria-label={t('speed')}>
      <button type="button" class:active={playbackSpeed === 'simulate'} aria-pressed={playbackSpeed === 'simulate'} on:click={() => setPlaybackSpeed('simulate')}>SIMULAR · 10s/ROUND</button>
      <button type="button" class:active={playbackSpeed === 'normal'} aria-pressed={playbackSpeed === 'normal'} on:click={() => setPlaybackSpeed('normal')}>{t('normal')} · 4×</button>
      <button type="button" class:active={playbackSpeed === 'fast'} aria-pressed={playbackSpeed === 'fast'} on:click={() => setPlaybackSpeed('fast')}>{t('fast')} · 8×</button>

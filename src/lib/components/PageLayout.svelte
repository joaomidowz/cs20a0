...
>
  {#if online && onlineNavigation}<OnlineNav {language} />{/if}
  <slot />
</main>
...
<style>
  ...
  @media(max-width:720px) {
    ...
    .online-surface { padding-top: 8px; }
    ...
    .online-surface :global(.topbar) { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; padding: 16px; }
    .online-surface :global(.topbar-actions) { grid-column: 1 / -1; justify-content: start; }

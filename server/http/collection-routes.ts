import { z } from 'zod';
import { DAILY_BASIC_PACKS, PACK_ODDS, PACK_PRICES } from '../../src/lib/game/online/collection-rules';
...
const buySchema = z.object({ tier: z.enum(['prata', 'ouro', 'era']), year: z.number().int().min(2013).max(2030).optional() });
...
export function createCollectionRoutes(db: Db, withAuth: (handler: Handler) => Handler): Route[] {
  return [
    ...
    route('GET', /^\/packs$/, withAuth(async ({ userId, now }) => {
      ...
      return { ok: true, today: view.packsToday, daily: DAILY_BASIC_PACKS, prices: PACK_PRICES, odds: PACK_ODDS, wallet: view.wallet };
    })),

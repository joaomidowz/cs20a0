...
import { upgradeCards } from '../collection/upgrader';
import type { Db } from '../db/client';
import { HttpError, readBody, route, type Handler, type Route } from './router';

const upgradeSchema = z.object({ stake: z.array(z.string().min(1).max(80)).min(1).max(UPGRADER_MAX_STAKE), target: z.string().min(1).max(80) });
...
export function createUpgraderRoutes(db: Db, withAuth: (handler: Handler) => Handler): Route[] {
  return [
    route('POST', /^\/upgrader$/, withAuth(async ({ request, userId }) => {
      ...
      return { ok: true, ...(await upgradeCards(db, userId!, body.stake, body.target).catch(toHttp)) };
    }))
  ];
}

...
describe.skipIf(!url)('coleção pela API (Postgres)', () => {
  ...
  it('venda paga 60% e some da coleção; comprar sem coins dá 402; comprar com coins funciona', async () => {
    ...
    expect(sold.body.coins).toBe(sellValue(playerById.get(playerId)!));
    expect((await call('/collection/sell', { playerId })).status).toBe(404);
    expect((await call('/packs/buy', { tier: 'ouro' })).status).toBe(402);
    await db.query(`INSERT INTO ledger (user_id, delta, reason) SELECT id, 5000, 'award' FROM users`);
    await db.query('UPDATE wallets SET coins = coins + 5000');
    const bought = await call('/packs/buy', { tier: 'era', year: 2019 });
    expect(bought.status).toBe(200);
    expect(bought.body.players.every((id: string) => playerById.get(id)?.year === 2019)).toBe(true);
  ...
  it('lineup exige cinco cartas próprias e devolve se o star vale', async () => {
    ...
    const collection = await call('/collection');
    const owned = (collection.body.players as Array<{ playerId: string }>).map((item) => playerById.get(item.playerId)!);
    ...
    const saved = await call('/lineup', { playerIds: five.map((player) => player.id), roles, starPlayerId: star.id, style: 'balanced' }, 'PUT');

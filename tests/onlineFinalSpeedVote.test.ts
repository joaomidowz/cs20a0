...
describe('voto por velocidade rápida na Grande Final', () => {
  ...
  it('só aplica Rápido após os dois finalistas votarem e rejeita não finalistas', () => {
    ...
    const roomMap = (manager as unknown as { rooms: Map<string, { phase: string; engine: unknown; live: Map<string, unknown>; config: RoomConfig; finalSpeedVotes: Set<string> }> }).rooms;
    ...
    expect(room.finalSpeedVotes).toEqual(new Set([p1]));
    ...
    expect(room.config.simulationSpeed).toBe('fast');
    expect(room.finalSpeedVotes).toEqual(new Set([p1, p2]));
  });
